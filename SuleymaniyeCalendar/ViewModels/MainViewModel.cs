...
namespace SuleymaniyeCalendar.ViewModels
{
	...
	public async void RefreshLocation()
	{
		...
		//{
			await GetPrayersAsync().ConfigureAwait(false);
			await Task.Run(async () =>
			{
				var location = new Location(_calendar.Latitude, _calendar.Longitude, _calendar.Altitude);
				_data.GetMonthlyPrayerTimes(location, true);
	...
	[RelayCommand]
	private async void PrayerSelected(Prayer prayer)
	{
		if (prayer == null)
			return;
	...
	public void OnAppearing()
	{
		...
		timer.Tick += (s,e) =>  RemainingTime = GetRemainingTime();
		timer.Start();
	}
